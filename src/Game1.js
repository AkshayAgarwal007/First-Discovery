...
FDOP.Game1.prototype = {
	create: function(){
        ...
        tree1 = this.add.sprite(1000,FDOP.GAME_HEIGHT-92-320,'tree');
    	tree2 = this.add.sprite(850,FDOP.GAME_HEIGHT-92-260,'tree');
    	tree1.scale.setTo(0.65,0.65);
    	tree2.scale.setTo(0.65,0.62);
    
    	platforms = this.add.group();
        platforms.enableBody = true;
        var ground = platforms.create(0, FDOP.GAME_HEIGHT - 100, 'layer4');
        ...
        player = this.add.sprite(250,FDOP.GAME_HEIGHT-220,'playernew');
        ...
        sprite = this.add.sprite(this.spawnx, FDOP.GAME_HEIGHT-510, 'bird');
        ...
        pear = this.add.sprite(sprite.position.x-61,FDOP.GAME_HEIGHT-500,'pear');
        ...
        dialog = this.add.sprite(sprite.position.x-60,FDOP.GAME_HEIGHT-595-10,'dialog');
    	dialog.scale.setTo(0.32,0.24);
        quesText = this.add.text(sprite.position.x-30,FDOP.GAME_HEIGHT-583-10, this.generateQuestion(), { fontSize: '45px', fill: '#000' });
        ...
        this.pickup_snd = this.add.audio('pickup');
        this.pickup_snd.volume=0.4;
    ...
    update: function(){
        ...
        if (sprite.position.x<this.game.world.bounds.right &&  sprite.position.x-100>this.game.world.bounds.left)
        this.register=true;
        else 
        this.register=false;
    ...
    createPlayerDialog: function() {
        ...
        xpos=55;
        playerDialog = this.add.sprite(player.position.x-xpos,player.position.y-105,'dialog');
        playerDialog.scale.setTo(0.31,0.38);
        txt = "Yummy! it's\ndelicious."
        playerText = this.add.text(player.position.x+30-xpos,player.position.y-95,txt, { fontSize: '45px', fill: '#000' });
        this.game.time.events.add(Phaser.Timer.SECOND * 6, this.playerDialogDestroy, this);
        this.add.tween(playerDialog).to( { alpha: 0 }, 5000, Phaser.Easing.Linear.None, true);
        this.add.tween(playerText).to( { alpha: 0 }, 5000, Phaser.Easing.Linear.None, true);    
    },
    
    playerDialogDestroy: function() {
    ...
    pauseAndPlay: function() {
        ...
        pause.visible=false;
        resume = this.add.sprite(FDOP.GAME_WIDTH/2, FDOP.GAME_HEIGHT/2, 'resume');
    ...
    replayGame: function()
    {
        ...
        pause.destroy();
        this.replay=true;
        resume = this.add.sprite(FDOP.GAME_WIDTH/2, FDOP.GAME_HEIGHT/2, 'replay');
        resume.scale.setTo(0.7,0.7);
        resume.anchor.setTo(0.5, 0.5);
        this.game.paused=true;
    ...
    unpause: function(event) {
        if (this.game.paused) {
            ...
            var x1 = FDOP.GAME_WIDTH/2-141, x2 = FDOP.GAME_WIDTH/2+141,
                y1 = FDOP.GAME_HEIGHT/2-141, y2 =FDOP.GAME_HEIGHT/2+141;
